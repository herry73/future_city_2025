...
def resident_air_panel(df):
    # -----------------------------------------------------
    # FINAL FIXED VERSION — Noise Hotspots Map with DATE + TIME
    # -----------------------------------------------------
    st.subheader("🗺 Noise Hotspots Map (Pick Day + Time)")

    # Use full dataset for timestamp selection (NOT df_range)
    df_full = df.copy()
    df_full.index = pd.to_datetime(df_full.index)

    # 1️⃣ Calendar – select a date
    all_dates = sorted(df_full.index.date)
    ...
    # 2️⃣ Time – select hour:minute
    selected_time = st.time_input(
        "Select time",
        value=datetime.strptime("12:00", "%H:%M").time()
    )

    # 3️⃣ Combine to timestamp
    selected_ts = pd.to_datetime(f"{selected_date} {selected_time}")

    # 4️⃣ Find closest timestamp in data
    df_full["ts_diff"] = (df_full.index - selected_ts).abs()
    closest_ts = df_full["ts_diff"].idxmin()

    # Extract that row
    df_point = df_full.loc[[closest_ts]]

    st.caption(f"Showing noise at closest available timestamp: **{closest_ts}**")
    ...
    if "latitude" not in df.columns or "longitude" not in df.columns:
        ...
        try:
            deck = map_layer(df_point, value_col="noise", color=[0, 80, 255])
            st.pydeck_chart(deck)
        except Exception as e:
            st.warning(f"Could not render noise map: {e}")
...
def planner_overview_page(df):
    ...
    # -----------------------------------------------------
    # FINAL NOISE MAP WITH DATE + TIME (THIS IS THE PART YOU WANT)
    # -----------------------------------------------------
    st.markdown("---")
    st.subheader("🗺 Noise Hotspots Map (Pick Day + Time)")
    ...
    # 3. Combine to timestamp
    selected_ts = pd.to_datetime(f"{selected_date} {selected_time}")

    # 4. Find closest timestamp
    df_full["ts_diff"] = (df_full.index - selected_ts).abs()
    closest_ts = df_full["ts_diff"].idxmin()
    df_point = df_full.loc[[closest_ts]]

    st.caption(f"Showing noise at closest timestamp: **{closest_ts}**")
