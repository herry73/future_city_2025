...
def controller_dashboard(df):
    ...
    if page == "Dashboard":
        ...
        controller_smart_alerts_page(df)
...
def controller_smart_alerts_page(df):
    st.subheader("Smart City Controller – Alerts & Recommendations")

    # Ensure health score column exists
    df = add_health_score(df.copy())

    # 1 — Health Score Plot (Plotly, nicer)
    st.markdown("### 💓 Air Quality Health Score")
    ...
    # 2 — Air Risk Alerts
    st.markdown("### 🔥 Air-Risk Alerts (Bad Air + High Noise)")
    ...
    if episodes.empty:
        ...
        df_alerts["air_risk_alert"].astype(int).plot(ax=ax2)
        ax2.set_title("Air-Risk Alerts (1 = Alert)")
        st.pyplot(fig2)

    # 3 — Night Noise Disturbances
    st.markdown("### 🌙 Night Noise Disturbances (22:00–06:00)")
    df_noise, night_events = detect_night_noise_events(
        df,
        noise_threshold=60,
    ...
    if night_events.empty:
        ...
        df_noise["noise_disturbance"].astype(int).plot(ax=ax3)
        ax3.set_title("Night Noise Disturbances (1 = Disturbance)")
        st.pyplot(fig3)

    # 4 — Tree Priority Index
    st.markdown("### 🌳 Tree Priority Score (Urban Green Planning)")
